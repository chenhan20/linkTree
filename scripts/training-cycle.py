...
def validate(plan):
    ...
    original = plan['days'].get(cycle.get('test_date'), {})
    reserve = plan['days'].get(cycle.get('rain_test_date'), {})
...
def block_view(plan, previous=None):
    ...
    return {'id': c['id'], 'title': c['title'], 'title_en': c['title_en'],
            ...
            'time_windows': {'tue': '早150分；晚籃球計入負荷', 'wed': '重訓／公司有氧，不騎車',
                             'thu': '早120分', 'fri_sat_sun_mon': '每次≤60分',
...
def render(plan):
    ...
    lines += ['', '表內時間只計騎車；週三0分不代表沒有重訓／公司課負荷。總時間包含往返／暖身／收操，不是工作段時間。戶外如遇交通延誤，先刪額外續騎；不為趕秒數冒險下坡。第一週腿疲勞或九月底週量偏低時，少做一堂週末輕鬆踩。',
              '', '## 執行與雨備', '']
    for date, raw in sorted(plan['days'].items()):
        if not raw['segments']:
            continue
...
def main():
    ...
    if args.command == 'choose':
        ...
        if args.variant == 'reserve_test' and plan['days'][plan['cycle']['test_date']]['selected_variant'] != 'rain':
            ap.error('Reserve test requires the original test to have been switched to rain recovery')
