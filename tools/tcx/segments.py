...
# 合理上限 = 路段長 / MIN_SPEED_MPS，再取下限 1800 s。
# 原本是 1.0 m/s（走路速度），對 3 公里的坡等於允許 54 分鐘 —— 於是「第一圈的起點」
# 會被配到「第二圈的終點」：至善路全段中位 15:43，卻出現 40:22 與 49:28 兩筆。
# 1.5 m/s（5.4 km/h）仍遠低於實測最慢的爬坡（碧山 9.1% 坡、2.2 m/s），不會誤殺真實成績。
MIN_SPEED_MPS = 1.5
