...
| [`data/movement-videos.json`](../data/movement-videos.json) | 178 KB | 375 支教學影片（125 個動作 × 3） | **延後**：第一次開教學或挑選器才抓 |
...
375 支（125 個動作 × 3），離線採集、人工複核後寫進 `data/movement-videos.json`。
...
第一批 360 支的 356 個不重複 ID、第二批 15 支、補件 1 支，全部回 200，0 個失效、0 個禁止嵌入。
第二批之後連標題與頻道名都跟宣稱的逐字對過。
...
用同一支通用影片會讓兩個動作在畫面上長得一模一樣。退掉之後那一格寫進 `issues`，檢查腳本一路點名到補件進來為止
（補的那支標題就叫 Soleus Calf muscle stretch，59 秒，沒有跟任何動作重複）。
...
收工畫面上有一顆「⧉ 複製今天的大綱」，一按就進剪貼簿，直接貼進 Strava 的說明欄。
...
· Dead bug 25 秒／側
· 無跳開合（Step Jack） 45 秒
× 2 組（動作間休 15 秒、大休 30 秒）

收操：最偉大伸展 20 秒／側、門框開胸 30 秒
```

分段課表會用段落標題，各段的輪數與休息寫在後面：
...
- 時間是**實際跑掉的時間**，不是估時
- 內容跟著跑的模式走：只做熱身就不列收操，只做收操就不列主課
- 沒有帶任何連結或出處 —— 這一頁對外是一般的訓練工具
- `navigator.clipboard` 失敗時（舊瀏覽器、非安全 context）會退回
  textarea ＋ `execCommand`；再失敗就把文字攤在畫面上讓你自己選
