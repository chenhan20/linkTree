...
| **Strava API** | ⚠️ 拿得到活動，但資料是**處理過**的；路段成績**需付費訂閱** |
...
5. **起跑取「最後一次」正向穿越，不是第一次。** 過了起點線又折返回來重新起跑時，
   前一次不算數。實測 2025-10-29 河濱10K：20:01:14 正向過線、20:02:57 反向折回、
   20:03:01 再正向起跑。取第一次會算成 1431 秒，取最後一次是 1324.6 秒 ——
   Strava 官方給的是 1324 秒。
6. 一趟多次通過各自成一筆。
...
對帳基準是 `scripts/harvest-strava.js` 封存的完整 Strava 資料）：
...
> 📌 **一個曾經寫錯的結論**：這份文件先前宣稱「自建偵測器抓到 9 筆 Strava 從未配對的成績」。
> **那是錯的。** 真正的原因是 `scripts/fetch-strava.js` 打 activity detail 時
> 沒有帶 `include_all_efforts=true`（該參數預設 false），Strava 只回「重點」efforts，
> 新路段沒有 PR 也沒被星號就整批不回傳。補上參數重抓之後，那 9 筆 Strava 全都有，
> 而且時間跟自建計時逐筆吻合。**Strava 沒有漏配對，是我們沒問對。**

另有 13 筆是 Strava 獨有的，但那些全都**早於本機 FIT 檔的起始日**（2025-08-13）——
是原始檔不存在，不是偵測失敗。`--compare` 會自動把這兩種情況分開列，
不然會把「檔案沒有」誤讀成「演算法有洞」。

**結論：自建計時與 Strava 官方在 91 筆上逐筆吻合，去 Strava 化不會失去任何資料。**
它的價值不是「比 Strava 準」，而是三件 Strava 給不了的事：
**不需要付費訂閱**（路段成績配對是付費功能）、**訂閱死了它還在跑**、
**改演算法可以重算全部歷史**（FIT 在本機，重掃不花任何 API 額度）。
...
| ITT 計時 | 要付費訂閱，訂閱斷了就沒了 | 自建偵測器，跟官方 91 筆逐筆吻合（平均差 0.75 秒） |
