...
- **全力預估影子**（`data/pacing.json`，scripts/build-pacing.py）：照 90 天功率曲線擬合的 CP／W′、最佳配速騎的那一隻，
  顏色 `--sea`；場景的第三種影子（`prof: {t, x}` 時間→距離表）。有同事的路段車道滿了就不加。
  第 05 章多一張「配速」：全力預估（海青）、最近一次（暮紫）、最快（日落橘）的逐 100 m 功率 —— 跟計時賽的影子同一組顏色
